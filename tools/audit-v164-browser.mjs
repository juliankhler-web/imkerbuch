...
function serverStarten() {
  const srv = createServer(async (req, res) => {
    ...
    try {
      ...
      let inhalt = await readFile(datei); if (poisonInstall && datei.endsWith('service-worker.js')) inhalt = Buffer.from(inhalt.toString().replace('imkerbuch-v183', 'imkerbuch-v183-auditinstall'));
...
async function main() {
 ...
 await schlaf(2000);
 results.push({id:'B8',name:'Neue Installation prüft die App-Hülle ebenfalls',...(await cdp.js(`const keys=await caches.keys();const c=keys.includes('imkerbuch-v183-auditinstall')?await caches.open('imkerbuch-v183-auditinstall'):null;const r=c?await c.match('./index.html'):null;return {cachedMaintenance:r?(await r.text()).includes('Audit-Wartungsseite'):null,keys};`))});
 offlineServer=true;
 await cdp.send('Network.emulateNetworkConditions',{offline:true,latency:0,downloadThroughput:0,uploadThroughput:0});
 await cdp.js('window.appReady=false');await cdp.send('Page.navigate',{url:base+'/index.html'});await ready();
 ...
 await writeFile(new URL('../docs/pruefung-v171/browser-results.json',import.meta.url),JSON.stringify({browser:browserKennung,date:new Date().toISOString(),results},null,2));
