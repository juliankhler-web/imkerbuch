...
const basis='55931e0';let phase=mode==='suite'?'neu':'alt';
...
try {
 ...
 await mkdir(resolve(root,'docs/pruefung-v171'),{recursive:true});
 await writeFile(resolve(root,`docs/pruefung-v171/${engine}-${mode}.json`),JSON.stringify({version,basis,engine,mode,datum:new Date().toISOString(),results:[{id:'BROWSER-START',status:'BLOCKED',detail:e.message}]},null,2)+'\n');
...
try{
 if(mode==='suite'){
  ...
  await page.goto(base+'index.html');await ready(page,'1.70');
  ...
  let before=await page.evaluate(async()=>{
   ...
   await S.set('wizardDone',true);await S.set('letzteGeseheneVersion','1.70');
  ...
  check('ALT-INSTALLIERT',before.caches.includes('imkerbuch-v182')&&before.db==='imkerbuch',{version:'1.70',cache:before.caches,db:before.db});
  ...
  check('DEFEKTES-UPDATE-ABGELEHNT',rejected.keys.includes('imkerbuch-v182')&&!rejected.keys.includes(cache)&&rejected.html.includes("const APP_VERSION = '1.70'"),{keys:rejected.keys});
  phase='aus';if(engine!=='webkit')await context.setOffline(true);await page.reload();await ready(page,'1.70');
  check('ALTE-APP-OFFLINE',await page.evaluate(()=>APP_VERSION==='1.70'),{version:'1.70'});
  ...
  check('NEUE-VERSION-AKTIV',after.version===version&&after.caches.includes(cache)&&!after.caches.includes('imkerbuch-v182'),{version:after.version,caches:after.caches});
 ...
 await mkdir(resolve(root,'docs/pruefung-v171'),{recursive:true});await writeFile(resolve(root,`docs/pruefung-v171/${engine}-${mode}.json`),JSON.stringify(out,null,2)+'\n');
