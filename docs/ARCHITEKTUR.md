...
|---|---|
| `ICONS_BUNT`, `iconBunt()` | **Bunte Bereichs-Icons** (24×24, gefüllte Flächen, Klasse `.ci`). Bewusst nur in Navigation und Bereichsauswahl verdrahtet: Seitenleiste, untere Leiste, „Alle Bereiche“, Dashboard-/Schnellzugriff-Konfiguration. Farben ausschließlich über die CSS-Klassen `a/ad/al/w/p/pn/g/gl/v/vl/b/bl/r/s/sl/k/d` → beide Themes schalten automatisch mit. Zwei Regeln beim Zeichnen: **keine** `id`-Attribute (dasselbe Icon steht mehrfach im DOM, doppelte Verlaufs-/clipPath-IDs würden sich überschreiben) und `.k` nur für dunkle Details **auf** hellen Flächen, `.d` für Silhouetten direkt auf der Kachel (wird im Dunkelmodus hell). Unbekannter Name → Rückfall auf `icon()`. Abgesichert durch 8 Tests in `tests/tests.js`. |
| `ICONS`, `icon()`, `logoSvg()`, `brandLogoSvg()` | Monochromes, **stroke-basiertes** Inline-SVG-Iconset (`fill="none"`, `stroke="currentColor"`) für alles Inhaltliche: Kartenköpfe, Buttons, Badges, Banner, Leerzustände. Bleibt monochrom, damit Farbe nicht mit Signalfarben (warn/danger/ok) und Amber-Buttons konkurriert. Dazu zwei Waben-Logos: `logoSvg()` (1 gefüllt + 2 Outline) = Default für PDFs/Onboarding/Einstellungen; `brandLogoSvg()` (3 gefüllte Amber-Waben, wie Play-Store, eigene Gradient-ID je Aufruf) = nur Homescreen-Header/Sidebar. `applyHeaderLogo()` zeigt aber immer zuerst ein eigenes Nutzer-Logo (`S.get('logo')` + `logoImHeader`). |
